...
const FRAME_INTERVAL_MS = 140; // ~7 fps. CPU JS is intentionally conservative.
...
async function ensureDetector() {
  ...
  if (
    ...
    throw new Error("TensorFlow.js face tracker scripts did not load.");
  }

  const backendReady = await tf.setBackend("cpu");
  if (!backendReady) {
    throw new Error("TensorFlow.js could not initialize its CPU backend.");
  }

  await tf.ready();

  if (tf.getBackend() !== "cpu") {
    throw new Error(`Unexpected TensorFlow.js backend: ${tf.getBackend()}`);
  }
  ...
  console.info(
    "Interactive Prop Lab tracker ready:",
...
function loop(now) {
  ...
  detector
    ...
    .then(handleFaces)
    .catch((error) => {
      console.error("CPU face inference failed:", error);
      failRuntime(error);
    })
    .finally(() => {
      detecting = false;
...
function handleFaces(faces) {
  ...
  const landmarks = face.keypoints.map((point) => ({
    x: point.x / width,
    y: point.y / height,
    z: (point.z ?? 0) / width,
  }));
