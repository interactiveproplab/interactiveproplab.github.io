...
const FACE_MESH_ROOT =
  `https://cdn.jsdelivr.net/npm/@mediapipe/face_mesh@${MEDIAPIPE_FACE_MESH_VERSION}`;
...
async function ensureFaceMesh() {
  ...
  if (typeof FaceMesh !== "function") {
    throw new Error("MediaPipe Face Mesh script did not load.");
  }
...
function loop(now) {
  ...
  faceMesh
    .send({ image: ui.video })
    .catch((error) => {
      console.error("Face Mesh inference failed:", error);
      failRuntime(error);
    })
    .finally(() => {
      sendingFrame = false;
...
function failRuntime(error) {
  ...
  showError(`Face tracking failed: ${error?.message || String(error)}`);
