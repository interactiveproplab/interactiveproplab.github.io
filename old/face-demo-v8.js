...
const FRAME_INTERVAL_MS = 55; // ~18 fps; plenty for the portfolio demo.
...
function ensureWorker() {
  ...
  workerInitPromise = new Promise((resolve, reject) => {
    if (!worker) {
      worker = new Worker(
        new URL("face-worker-v8.js", import.meta.url),
        { type: "module" }
      );
      ...
      worker.addEventListener("error", (event) => {
        ...
        reject(new Error(event.message || "Face tracking worker failed to load."));
      });
    }
    ...
    const initialized = (event) => {
      if (event.data?.type === "READY") {
        ...
        worker.removeEventListener("message", initialized);
        workerReady = true;
        workerInitPromise = null;
        resolve();
      }
      ...
      if (event.data?.type === "INIT_ERROR") {
        ...
        worker.removeEventListener("message", initialized);
        workerInitPromise = null;
        reject(new Error(event.data.error || "Face tracker initialization failed."));
      }
    };

    worker.addEventListener("message", initialized);
...
async function loop(now) {
  ...
  try {
    ...
    const timestampMs = Math.max(rawTimestamp, lastTimestampMs + 1);
    ...
    console.error("Could not create camera frame:", error);
    failRuntime(error);
  }
}
...
function onWorkerMessage(event) {
  ...
  if (message.type === "DETECT_ERROR") {
    workerBusy = false;
    console.error("Face Landmarker detection error:", message.error);
    failRuntime(new Error(message.error || "Face detection failed."));
    return;
  }
